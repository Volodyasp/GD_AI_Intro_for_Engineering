...
class GuardrailsManager:
    def __init__(self):
        ...
        self.model_name = CONFIG["vertex_ai"]["models"]["generate_data"].get(
            "model_name", "gemini-2.0-flash"
        )

        self.model = GenerativeModel(
            model_name=self.model_name, system_instruction=GUARDRAILS_SYSTEM_INSTRUCTION
        )
    ...
    @trace_step(name="guardrails_check", as_type="generation")
    async def check_input(self, user_prompt: str) -> GuardrailsResult:
        ...
        try:
            ...
            response = await self.model.generate_content_async(
                user_prompt, generation_config=self.config
            )
            ...
            logger.info(
                f"Guardrails Result: Safe={guard_result.is_safe}, Relevant={guard_result.is_relevant}"
            )
            return guard_result
